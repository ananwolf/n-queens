...
window.findSolution = function(row, n, board, validator, callback) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    // base case we only check for row conflict,
    if (!board[validator]()) {
      var result = findSolution(row + 1, n, board, validator, callback);
      if (result) {
        return result;
      }
      // var copyBoard = new Board({n: n});
      // for (var i = 0; i < n; i++) {}
      // solveNRooks(n, rowIndex + 1, board, possibleSolutions);
      // if (solution(board, colIndex+1) === true)
    }
    board.togglePiece(row, i);
    // possibleSolutions.push(board.get(rowIndex));
  }
